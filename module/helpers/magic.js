function getPenetrationAbilityDetails(actorData) {
  const penetrationId =
    actorData.data.data?.laboratory?.abilitiesSelected?.penetration
      ?.abilityID || "";
  if (penetrationId) {
...
function getParmaAbilityDetails(actorData) {
  const parmaId =
    actorData.data.data?.laboratory?.abilitiesSelected?.parma?.abilityID || "";
  if (parmaId) {
    ...
    if (parmaAbility) {
      return {
        score: parmaAbility.data.derivedScore,
...
function calculatePenetration({ actorCaster, roll, form, technique }) {
  ...
  if (
    form.toUpperCase() === speciality.toUpperCase() ||
    technique.toUpperCase() === speciality.toUpperCase()
  ) {
    penetration += 1;
...
function calculateResistance(actorData, form) {
  let magicResistance =
    Number(actorData.data.data.laboratory?.magicResistance?.value) ||
    Number(actorData.data.data?.might?.value) ||
    0;
  let specialityIncluded = "";
  const parma = getParmaAbilityDetails(actorData);
  if (
    parma.specialityIncluded &&
    parma.specialityIncluded.toUpperCase() === form.toUpperCase()
  ) {
  ...
  if (arts) {
    ...
    formScore = arts.forms[formKey].derivedScore || 0;
  }
